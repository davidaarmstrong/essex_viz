...
pd <- reactive({
 ...
 xt.steps <- reactive({
   data.frame(
     ...
     intro = c(
       ...
       "When there is no column variable, you see a frequency/relative frequency distribution of the selected variable. The entries are counts and column proportions.", 
       ...
       "Finally, when you choose a column varaible, you also get a mosaic plot.  This is a visual representation of the contingency table.  The width of each column is proportional to the number in each column-variable group.  The height of each bar within the column is a representation of the column proportion."
     ), 
 ...
 model.steps <- reactive({
   data.frame(
   element = c("#dv", "#mod1 + .selectize-control", "#subset", "#mo1", "#mo2", "#varinst",  "#p1", "#t1a", "#t1b", "#mod2 + .selectize-control", "#modOutput", "#varinst", "#p1", "#btnOutput", "#colPickers", "#legPos", "#plotSizeCtrl", "#xto", "#yto", "#rotlabs", "#axTitles", "#varby", "#p1", "#modOutput","#pp1a", "#pp1b"),
   intro = c("First, pick a dependent variable to model.  These include votes for specific parties, for the incumbent and vote turnout.  Pick one now to see how it works.",
             ...
             "The main entries in the table are logistic regression coefficients calculated using survey weights to produce nationally representative predictions.  The standard errors of the coefficients are in parentheses.  These coefficients help us understand which effects are statistically reliable.  However, there are better ways to understand the substantive impact of a variable.  Hit 'Next' to find out how.",
             "This table gives you model fit statistics.  The Proportional Reduction in Error (PRE) tells you by what proportion you reduce prediction errors by your model.  The expected PRE is similar, but takes into account the size of the predicted probabilities.  These values have an upper bound of 1, but are not bounded on the lower end at 0.  Numbers 0 or less mean you are better of with no model rather than the one you estimated",
             "You can pick an independent variable from this list to learn more about its effect on the dependent variable.  The effects do not show up here, but in other tabs.  If you have estimated only one model, this list is automatically populated with all of the variables in the model.  Choose a variable and hit the 'Next' button to see them.",
             "The plot that is generated here shows the predicted probability of engaging in the dependent variable activity (turnout, vote incumbent or vote for a particular party) for each of the values of the variable you just chose on the previous page.  The points represent the probabilities and the lines give the 95% confidence interval for the probability.  See the app's documentation for more details on how this was calculated.  There are some controls for the plot, but we'll talk more about them in a minute.  Hit the 'Next' button to see more results.  The plot is made with plotly, so hovering your mouse over the top of the plot will activat lots of interactive options including exporting it as a .png file. ",
             ...
             "Clicking this button will make available a number of different controls for the plot visible. Given all of the different options, it is difficult to make it look perfect automatically, so there are lots of controls here to help you make the plot look the way you want.  Click the 'Show Plot Controls' buton now; you will be guided through the different controls when you hit 'Next' in the tooltip.",
             "These are colour pickers for the points and lines from the first and models.  By clicking this, you can interactively define the colour and the alpha channel for the transparency.  Give it a try!",
             ...
             "These input boxes allow you to move the title on the x-axis in the vertical (y) and horizontal (x) directions.",
             "These input boxes allow you to move the title on the y-axis in the vertical (y) and horizontal (x) directions.",
             ...
             "These are again the predicted probabilities, but this time they are for every combination of independent variable of interest and either year or region, depending on which one you chose.",
             "The differences in predicted probability table shows the difference in predicted probability for every combination of variable of interest and region or year and every other combination of those same two variables.  You'll notice that there is a 'page' selector at the bottom of the table as the table can be quite long. The boxes underneath the table headings can be used to filter the results.  By clicking in the boxes, you can select the elements you want to keep (for categorical variables) or you can specify a range for continuous variables."),
   position = c("right", "right", "right", "top", "top", "right", "right", "left", "top", "right", "top", "right", "right", "top", "right", "right", "right", "right", "right", "right", "right", "right", "right", "right", "bottom", "top"))
 })
