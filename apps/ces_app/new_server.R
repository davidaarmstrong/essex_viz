...
## change catholic to be a factor. check variable spelling. 
## fix models 
...
dv.names <- c("Vote Liberal"  = "vote_lib", "Vote Conservative" = "vote_con", 
              ...
              "Vote Green" = "vote_green", "Vote Turnout" = "turnout")
blocks <- list()
...
blocks[[5]] <- c("sp_defence", "sp_envir", "immig", "usties", "jobspriv", 
                 "blame", "poorgap", "stayhome", "dowomen", "feelgays")
...
val_num_names <- c(" ", "Market Liberalism", "Moral Traditionalism", "Political Cynicism", "Continentalism", 
                   ...
                   "Feeling Thermometer: Gays/Lesbians")
...
#     Add to your server
  ...
  c1 <- reactive({
    ...
    out <- probci(m1(), tmpdat()[[1]]$variables, changeX=cx[[input$varby]], .b=m1a()$res$coefficients, .vcov=m1a()$res$variance, numQuantVals=2, type="aveCase") 
  ...
  c2 <- reactive({
    req(m2())
    cx <- list("none" = input$focus_var, "year" = c("year_fac", input$focus_var), "region" = c("province", input$focus_var))
    out <- probci(m2(), tmpdat()[[1]]$variables, changeX=cx[[input$varby]], .b=m2a()$res$coefficients, .vcov=m2a()$res$variance, numQuantVals=2, type="aveCase") 
    out
  })
  c1a <- reactive({
    req(m1())
    cx <- list("none" = input$focus_var, "year" = c("year_fac", input$focus_var), "region" = c("province", input$focus_var))
    out <- probci2(m1(), tmpdat()[[1]]$variables, changeX=cx[[input$varby]], .b=m1a()$res$coefficients, .vcov=m1a()$res$variance, numQuantVals=2, type="aveCase") 
    out
    })
  c2a <- reactive({
    req(m2())
    cx <- list("none" = input$focus_var, "year" = c("year_fac", input$focus_var), "region" = c("province", input$focus_var))
    out <- probci2(m2(), tmpdat()[[1]]$variables, changeX=cx[[input$varby]], .b=m2a()$res$coefficients, .vcov=m2a()$res$variance, numQuantVals=2, type="aveCase") 
    out
  })
  d1 <- reactive({
    req(c1a())
